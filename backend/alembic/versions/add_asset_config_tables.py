...
def upgrade():
    ...
    op.create_table('asset_configs',
        ...
        sa.Column('asset_type', sa.Enum('A_SHARE', 'US_STOCK', 'HK_STOCK', 'CRYPTO', 'FUTURES', 'OPTIONS', 'BONDS', 'FUNDS', name='assettype'), nullable=False),
        sa.Column('name', sa.String(length=100), nullable=False, comment='资产类型名称'),
        ...
        sa.Column('supported_functions', sa.JSON(), nullable=False, comment='支持的功能列表'),
        ...
        sa.Column('status', sa.Enum('ACTIVE', 'INACTIVE', 'MAINTENANCE', name='assetconfigstatus'), nullable=False),
    ...
    op.create_index(op.f('ix_asset_configs_asset_type'), 'asset_configs', ['asset_type'], unique=True)
    op.create_index(op.f('ix_asset_configs_id'), 'asset_configs', ['id'], unique=False)

    # Create asset_symbols table
    op.create_table('asset_symbols',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('asset_type', sa.Enum('A_SHARE', 'US_STOCK', 'HK_STOCK', 'CRYPTO', 'FUTURES', 'OPTIONS', 'BONDS', 'FUNDS', name='assettype'), nullable=False),
        ...
        sa.Column('full_name', sa.String(length=500), nullable=True, comment='完整名称'),
        ...
        sa.Column('tick_size', sa.String(length=20), nullable=True, comment='最小价格变动单位'),
        sa.Column('is_active', sa.Boolean(), nullable=False, comment='是否活跃'),
        sa.Column('is_tradable', sa.Boolean(), nullable=False, comment='是否可交易'),
        sa.Column('listing_date', sa.DateTime(timezone=True), nullable=True, comment='上市日期'),
        sa.Column('delisting_date', sa.DateTime(timezone=True), nullable=True, comment='退市日期'),
        sa.Column('metadata', sa.JSON(), nullable=True, comment='扩展元数据'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_asset_symbols_asset_type'), 'asset_symbols', ['asset_type'], unique=False)
    op.create_index(op.f('ix_asset_symbols_id'), 'asset_symbols', ['id'], unique=False)
    op.create_index(op.f('ix_asset_symbols_symbol'), 'asset_symbols', ['symbol'], unique=False)
    ...
    op.create_table('asset_market_data',
        ...
        sa.Column('change_amount', sa.String(length=20), nullable=True, comment='涨跌额'),
        sa.Column('change_percent', sa.String(length=10), nullable=True, comment='涨跌幅'),
        ...
        sa.Column('dividend_yield', sa.String(length=10), nullable=True, comment='股息率'),
        sa.Column('open_interest', sa.String(length=30), nullable=True, comment='持仓量'),
        sa.Column('settlement_price', sa.String(length=20), nullable=True, comment='结算价'),
        sa.Column('implied_volatility', sa.String(length=10), nullable=True, comment='隐含波动率'),
        sa.Column('market_cap_rank', sa.Integer(), nullable=True, comment='市值排名'),
        sa.Column('circulating_supply', sa.String(length=30), nullable=True, comment='流通供应量'),
        sa.Column('total_supply', sa.String(length=30), nullable=True, comment='总供应量'),
        sa.Column('trade_date', sa.DateTime(timezone=True), nullable=False, comment='交易日期'),
        sa.Column('data_timestamp', sa.DateTime(timezone=True), nullable=True, comment='数据时间戳'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_asset_market_data_asset_type'), 'asset_market_data', ['asset_type'], unique=False)
    op.create_index(op.f('ix_asset_market_data_id'), 'asset_market_data', ['id'], unique=False)
    op.create_index(op.f('ix_asset_market_data_symbol'), 'asset_market_data', ['symbol'], unique=False)
    op.create_index(op.f('ix_asset_market_data_trade_date'), 'asset_market_data', ['trade_date'], unique=False)

    # Create asset_screener_templates table
    op.create_table('asset_screener_templates',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('asset_type', sa.Enum('A_SHARE', 'US_STOCK', 'HK_STOCK', 'CRYPTO', 'FUTURES', 'OPTIONS', 'BONDS', 'FUNDS', name='assettype'), nullable=False),
    ...
    op.create_index(op.f('ix_asset_screener_templates_asset_type'), 'asset_screener_templates', ['asset_type'], unique=False)
    op.create_index(op.f('ix_asset_screener_templates_id'), 'asset_screener_templates', ['id'], unique=False)

    # Create asset_backtest_templates table
    op.create_table('asset_backtest_templates',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('asset_type', sa.Enum('A_SHARE', 'US_STOCK', 'HK_STOCK', 'CRYPTO', 'FUTURES', 'OPTIONS', 'BONDS', 'FUNDS', name='assettype'), nullable=False),
        sa.Column('name', sa.String(length=100), nullable=False, comment='模板名称'),
        sa.Column('description', sa.Text(), nullable=True, comment='模板描述'),
        sa.Column('strategy_type', sa.String(length=50), nullable=False, comment='策略类型'),
    ...
    op.create_index(op.f('ix_asset_backtest_templates_asset_type'), 'asset_backtest_templates', ['asset_type'], unique=False)
    op.create_index(op.f('ix_asset_backtest_templates_id'), 'asset_backtest_templates', ['id'], unique=False)


def downgrade():
    # Drop tables in reverse order
    op.drop_index(op.f('ix_asset_backtest_templates_id'), table_name='asset_backtest_templates')
    op.drop_index(op.f('ix_asset_backtest_templates_asset_type'), table_name='asset_backtest_templates')
    op.drop_table('asset_backtest_templates')
    
    op.drop_index(op.f('ix_asset_screener_templates_id'), table_name='asset_screener_templates')
    op.drop_index(op.f('ix_asset_screener_templates_asset_type'), table_name='asset_screener_templates')
    op.drop_table('asset_screener_templates')
    
    op.drop_index(op.f('ix_asset_market_data_trade_date'), table_name='asset_market_data')
    ...
    op.drop_index(op.f('ix_asset_market_data_asset_type'), table_name='asset_market_data')
