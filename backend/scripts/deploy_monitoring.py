...
class MonitoringDeployer:
    ...
    def _prepare_environment(self):
        ...
        # 设置目录权限
        subprocess.run(
            ["sudo", "chown", "-R", "65534:65534", self.config.prometheus_data_dir],
            check=True,
        )
        subprocess.run(
            ["sudo", "chown", "-R", "472:472", self.config.grafana_data_dir], check=True
        )
        ...
        try:
            ...
            result = subprocess.run(
                [
                    ...
                    "ls",
                    "--filter",
                    f"name={self.config.network_name}",
                    "--format",
                    "{{.Name}}",
                ],
                check=False,
    ...
    def _deploy_prometheus(self):
        ...
        cmd = [
            ...
            "-v",
            f"{self.config_dir}/alert-rules.yml:/etc/prometheus/alert-rules.yml",
            "-v",
            f"{self.config.prometheus_data_dir}:/prometheus",
            "--user",
            "65534:65534",
    ...
    def _deploy_grafana(self):
        ...
        cmd = [
            ...
            self.config.network_name,
            "-p",
            f"{self.config.grafana_port}:3000",
            "-v",
            f"{self.config.grafana_data_dir}:/var/lib/grafana",
            "-e",
            f"GF_SECURITY_ADMIN_USER={self.config.grafana_admin_user}",
            "-e",
            f"GF_SECURITY_ADMIN_PASSWORD={self.config.grafana_admin_password}",
            "-e",
    ...
    def _deploy_alertmanager(self):
        ...
        cmd = [
            ...
            self.config.network_name,
            "-p",
            f"{self.config.alertmanager_port}:9093",
            "-v",
        ...
        # 等待服务启动
        self._wait_for_service(
            f"http://localhost:{self.config.alertmanager_port}/-/healthy",
            "Alertmanager",
        )
    ...
    def _deploy_node_exporter(self):
        ...
        cmd = [
            ...
            self.config.network_name,
            "-p",
            f"{self.config.node_exporter_port}:9100",
            "--pid",
            "host",
            "-v",
            "/:/host:ro,rslave",
    ...
    def _deploy_postgres_exporter(self):
        ...
        cmd = [
            ...
            self.config.network_name,
            "-p",
            f"{self.config.postgres_exporter_port}:9187",
            "-e",
            f"DATA_SOURCE_NAME={db_url}",
    ...
    def _deploy_redis_exporter(self):
        ...
        cmd = [
            ...
            self.config.network_name,
            "-p",
            f"{self.config.redis_exporter_port}:9121",
    ...
    def _deploy_nginx_exporter(self):
        ...
        cmd = [
            ...
            self.config.network_name,
            "-p",
            f"{self.config.nginx_exporter_port}:9113",
    ...
    def _add_prometheus_datasource(self):
        ...
        try:
            ...
            if response.status_code in [200, 409]:  # 200: 成功, 409: 已存在
                ...
                logger.warning(
                    f"数据源配置响应: {response.status_code} - {response.text}"
                )
    ...
    def _import_dashboard(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"仪表板导入响应: {response.status_code} - {response.text}"
                )
    ...
    def _verify_deployment(self):
        ...
        services = [
            ...
            (
                f"http://localhost:{self.config.node_exporter_port}/metrics",
                "Node Exporter",
            ),
        ]
...
def main():
    ...
    parser.add_argument(
        "action", choices=["deploy", "stop", "restart"], help="操作类型"
    )
