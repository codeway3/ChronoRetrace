...
class MonitoringHealthChecker:
    ...
    def _get_service_uptime(self, service_name: str) -> float:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                uptime = (datetime.now().replace(tzinfo=start_time.tzinfo) - start_time).total_seconds() / 3600
    ...
    def _check_alert_conditions(self, service_key: str, service_config: Dict, status: ServiceStatus):
        """检查告警条件"""
        failure_count = self.failure_counts[service_key]
        
        # 检查是否达到告警阈值
        if (failure_count >= self.config.failure_threshold and 
            status.status in ["unhealthy", "unknown"]):
            ...
            if (last_alert is None or 
                (now - last_alert).total_seconds() > 3600):  # 1小时内不重复告警
    ...
    def _format_alert_message(self, service_name: str, status: ServiceStatus, failure_count: int) -> str:
        """格式化告警消息"""
        return f"""
🚨 ChronoRetrace监控告警
...
请立即检查服务状态并采取相应措施。
    ...
    def _print_status_summary(self, results: Dict[str, ServiceStatus], overall_healthy: bool):
        """打印状态摘要"""
        print("\n" + "="*60)
        ...
        for service_key, status in results.items():
            ...
            print(f"{status_icon} {status.name:<20} {status.status:<10} {status.response_time:.2f}s")
