...
def test_get_futures_data_success(mock_fetch):
    ...
    successful_mock.return_value = pd.DataFrame({
        'trade_date': ['2023-01-01'], 'close': [4010.0], 'open': [1.0], 'high': [1.0], 'low': [1.0], 'vol': [1.0]
    })
...
@patch('app.services.futures_fetcher.fetch_futures_from_yfinance')
def test_get_futures_data_not_found(mock_fetch):
    mock_fetch.return_value = pd.DataFrame()
    response = client.get("/api/v1/futures/UNKNOWN")
...
async def test_get_futures_list_success(mock_run_in_threadpool):
    mock_run_in_threadpool.return_value = pd.DataFrame({
        'symbol': ['rb2410', 'au2412'], 'name': ['螺纹钢2410', '黄金2412']
    })
