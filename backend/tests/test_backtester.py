...
def test_grid_backtest_api_endpoint(mocker, mock_stock_data):
    ...
    request_payload = {
        "stock_code": "API.TEST.SH", "start_date": "2023-01-01", "end_date": "2023-01-08",
        "upper_price": 11.0, "lower_price": 10.0, "grid_count": 2, "total_investment": 20000.0,
        "commission_rate": 0, "min_commission": 0, "stamp_duty_rate": 0,
    }
...
def test_backtest_api_no_data_error(mocker):
    ...
    request_payload = {"stock_code": "NO.DATA", "start_date": "2023-01-01", "end_date": "2023-01-08", "upper_price": 11.0, "lower_price": 10.0, "grid_count": 2, "total_investment": 1000.0}
    response = client.post("/api/v1/backtest/grid", json=request_payload)
...
def test_backtest_api_no_data_in_range_error(mocker, mock_stock_data):
    ...
    mocker.patch('app.services.data_fetcher.fetch_stock_data', return_value=mock_stock_data)
    request_payload = {"stock_code": "OUT.OF.RANGE", "start_date": "2024-01-01", "end_date": "2024-01-08", "upper_price": 11.0, "lower_price": 10.0, "grid_count": 2, "total_investment": 1000.0}
    response = client.post("/api/v1/backtest/grid", json=request_payload)
    assert response.status_code == 400
