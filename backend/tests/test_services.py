...
def test_update_stock_list_from_akshare(mock_bj, mock_sz, mock_sh, db_session, mock_akshare_data):
...
def test_update_us_stock_list(mock_other, mock_dow, mock_nasdaq, mock_sp500, db_session):
...
def test_clear_all_financial_data(db_session):
    ...
    try:
        actions = [{"action_type": "dividend", "ex_date": date(2023, 6, 1), "value": 0.5}]
    ...
    try:
        with verify_session.connection() as conn:
            assert conn.execute(text("SELECT COUNT(*) FROM fundamental_data")).scalar_one() == 1
            assert conn.execute(text("SELECT COUNT(*) FROM corporate_actions")).scalar_one() == 1
            assert conn.execute(text("SELECT COUNT(*) FROM annual_earnings")).scalar_one() == 1
    finally:
    ...
    try:
        with final_verify_session.connection() as conn:
            assert conn.execute(text("SELECT COUNT(*) FROM fundamental_data")).scalar_one() == 0
            assert conn.execute(text("SELECT COUNT(*) FROM corporate_actions")).scalar_one() == 0
            assert conn.execute(text("SELECT COUNT(*) FROM annual_earnings")).scalar_one() == 0
    finally:
...
def test_store_stock_data_upsert(db_session, mock_kline_data):
    ...
    # 4. Verify results
    with db_session.connection() as conn:
        result = conn.execute(text("SELECT * FROM stock_data ORDER BY trade_date")).fetchall()
        assert len(result) == 3
        # Check updated row
...
def test_store_fundamental_data(db_session):
    ...
    try:
        ...
        with new_session.connection() as conn:
            result_updated = conn.execute(text("SELECT * FROM fundamental_data")).fetchone()
...
def test_store_annual_earnings(db_session):
    ...
    try:
        ...
        with new_session.connection() as conn:
            result = conn.execute(text("SELECT * FROM annual_earnings ORDER BY year")).fetchall()
            assert len(result) == 3
...
def test_fetch_from_db(db_session, mock_stock_data_db):
    ...
    fetcher = StockDataFetcher(db=db_session, stock_code='TEST.SH', interval='daily', market_type='A_share')
...
def test_fetch_from_api_when_db_empty(mock_fetch_a_share, db_session, mock_stock_data_api):
    ...
    db_df = pd.read_sql(text("SELECT * FROM stock_data WHERE ts_code='TEST.SH'"), db_session.connection())
...
def test_fetch_from_db_when_data_is_fresh(db_session, mock_stock_data_db):
    ...
    # 2. Action
    with patch('app.services.a_share_fetcher.fetch_a_share_data_from_akshare') as mock_fetch:
        fetcher = StockDataFetcher(db=db_session, stock_code='TEST.SH', interval='daily', market_type='A_share')
        df = fetcher.fetch_stock_data()
...
@patch('app.services.a_share_fetcher.fetch_a_share_data_from_akshare')
def test_fetch_from_api_when_db_stale(mock_fetch_a_share, db_session, mock_stock_data_db, mock_stock_data_api):
...
def test_fetch_us_stock_directly_from_api(mock_fetch_us, db_session, mock_stock_data_api):
    """Tests that US stock data is fetched from the correct (yfinance) API."""
    # 1. Setup
    mock_fetch_us.return_value = mock_stock_data_api

    # 2. Action
    fetcher = StockDataFetcher(db=db_session, stock_code='AAPL', interval='daily', market_type='US_stock')
    df = fetcher.fetch_stock_data()
...
@patch('app.services.a_share_fetcher.fetch_a_share_data_from_akshare')
def test_fetch_minute_data_bypasses_db(mock_fetch_a_share, db_session, mock_stock_data_api):
    ...
    with patch.object(StockDataFetcher, '_fetch_from_db') as mock_fetch_db:
        fetcher = StockDataFetcher(db=db_session, stock_code='TEST.SH', interval='minute', market_type='A_share')
        df = fetcher.fetch_stock_data()
