...
def downgrade(engine):
    ...
    with engine.connect() as conn:
        # 删除创建的索引
        try:
            conn.execute(text("DROP INDEX CONCURRENTLY IF EXISTS idx_stock_data_ts_code_date_partial"))
            conn.execute(text("DROP INDEX CONCURRENTLY IF EXISTS idx_users_fulltext_search"))
            print("✅ 自定义索引已删除")
        except Exception as e:
            print(f"⚠️ 索引删除失败: {e}")
