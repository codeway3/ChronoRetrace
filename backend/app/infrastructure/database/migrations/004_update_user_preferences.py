...
def downgrade(engine):
    ...
    with engine.connect() as conn:
        # 删除新添加的字段
        try:
            conn.execute(text("ALTER TABLE user_preferences DROP COLUMN IF EXISTS currency"))
            conn.execute(text("ALTER TABLE user_preferences DROP COLUMN IF EXISTS default_chart_type"))
            conn.execute(text("ALTER TABLE user_preferences DROP COLUMN IF EXISTS default_period"))
            conn.execute(text("ALTER TABLE user_preferences DROP COLUMN IF EXISTS preferred_indicators"))
            conn.execute(text("ALTER TABLE user_preferences DROP COLUMN IF EXISTS risk_tolerance"))
            conn.execute(text("ALTER TABLE user_preferences DROP COLUMN IF EXISTS investment_goal"))
            conn.execute(text("ALTER TABLE user_preferences DROP COLUMN IF EXISTS investment_horizon"))
            print("✅ 新添加的字段已删除")
        except Exception as e:
            print(f"⚠️ 删除新字段失败: {e}")
