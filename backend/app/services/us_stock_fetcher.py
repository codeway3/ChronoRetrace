...
def fetch_from_yfinance(ts_code: str, start_date: str, end_date: str, interval: str = "1d") -> pd.DataFrame:
    """
    Fetches data from yfinance and formats it to match Tushare's output.
    """
    yf_ticker = _convert_ts_code_to_yfinance(ts_code)
    logger.info(f"Fetching yfinance data for ticker: {yf_ticker}, interval: {interval}, start: {start_date}, end: {end_date}")
    ...
    logger.info(f"Successfully fetched {len(df)} rows from yfinance for {yf_ticker}. Original Columns: {df.columns.tolist()}")
    logger.debug(f"First 5 rows of original yfinance data for {yf_ticker}:\n{df.head().to_string()}")
    ...
    if df.empty:
        logger.warning(f"DataFrame became empty after dropping rows with missing dates for {yf_ticker}")
        return pd.DataFrame()
    ...
    numeric_cols = ['open', 'high', 'low', 'close', 'vol', 'pre_close', 'change', 'pct_chg']
    ...
    logger.debug(f"First 5 rows of final DataFrame for {yf_ticker}:\n{df.head().to_string()}")
...
def update_us_stock_list(db: Session):
    ...
    try:
        ...
        logger.error(f"Error fetching US stock symbols with yahoo_fin: {e}", exc_info=True)
        return
    ...
    if stocks_to_insert:
        ...
        try:
            db.execute(stmt)
            db.commit()
            logger.info(f"Successfully upserted {len(stocks_to_insert)} US stock entries into DB.")
        except Exception as e:
