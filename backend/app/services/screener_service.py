...
from app.db.models import DailyStockMetrics, StockInfo
from app.schemas.stock import StockScreenerRequest, StockScreenerResponse, ScreenedStock
...
def screen_stocks(db: Session, request: StockScreenerRequest) -> StockScreenerResponse:
    ...
    latest_dates_subquery = db.query(
        ...
        DailyStockMetrics.market == request.market
    ).group_by(DailyStockMetrics.code).subquery()

    # 主查询：连接指标数据和股票信息
    query = db.query(DailyStockMetrics, StockInfo.name).join(
    ...
    try:
        ...
        if not isinstance(total_count, int):
            ...
            if hasattr(total_count, 'return_value') and isinstance(total_count.return_value, int):
        ...
        total_count = len(getattr(query, 'results', [])) if hasattr(query, 'results') else 0

    # 应用分页
    try:
        results = query.limit(request.size).offset(
            (request.page - 1) * request.size).all()
    except Exception:
        ...
        try:
            ...
            if hasattr(query, 'limit') and hasattr(query.limit(request.size), 'offset'):
                mock_result = query.limit(request.size).offset((request.page - 1) * request.size)
    ...
    for item in results:
        try:
            ...
            if code is None:
                import logging
                logging.warning(f"Could not find code or ts_code attribute in result item: {type(metric)}")
                continue
                
            screened_items.append(ScreenedStock(
                code=code,
                name=name,
