...
def _normalize_df_columns(df: pd.DataFrame, column_map: Dict[str, str]) -> pd.DataFrame:
    """Rename columns based on a map."""
...
def _normalize_hist_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = _normalize_df_columns(df, column_map)

    required = ["trade_date", "open", "high", "low", "close"]
    if not all(col in df.columns for col in required):
        logger.warning(f"hist df missing one of required columns: {required}. Got: {df.columns.tolist()}")
        return pd.DataFrame()
...
def fetch_industry_hist(industry_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"Fetching Eastmoney hist for industry: '{industry_name}' until {today}")
        ...
        logger.error(f"Failed to fetch Eastmoney hist for '{industry_name}': {exc}", exc_info=True)
        return pd.DataFrame()
...
@cache(expire=3600)
def build_overview(window: str = "20D", provider: str = "em") -> List[Dict[str, object]]:
...
def fetch_industry_constituents(industry_code: str) -> List[Dict[str, object]]:
    ...
    try:
        ...
        column_map = {
            ...
            "市盈率-动态": "pe_ratio", "换手率": "turnover_rate",
        }
        df = _normalize_df_columns(df, column_map)
        
        required = ["stock_code", "stock_name"]
        if not all(col in df.columns for col in required):
            logger.error(f"Missing required columns in constituents df. Got: {df.columns.tolist()}")
            return []
