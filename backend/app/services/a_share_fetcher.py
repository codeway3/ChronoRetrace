...
def _baostock_query_with_retry(query_func, *args, **kwargs):
    ...
    for attempt in range(max_retries):
            ...
            try:
                ...
                if "columns passed" in str(e) and rs.data:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Using first {num_data_cols} fields out of {len(rs.fields)} available: {limited_fields}")
        ...
        if rs.error_code == '10002007':  # Network reception error
            ...
            logger.error(
                f"Baostock query failed with code {rs.error_code}: {rs.error_msg}")
            return None

    logger.error(
        f"Failed to execute Baostock query after {max_retries} retries.")
    return None
...
def update_stock_list_from_akshare(db: Session):
    ...
    for market, (func, suffix, code_col) in stock_map.items():
        try:
            ...
            df.rename(columns={code_col: "代码", "名称": "name",
        ...
        if df_dividends is not None and not df_dividends.empty:
            for _, row in df_dividends.iterrows():
                try:
                    ...
                    dividend_str = str(row["dividCashPsAfterTax"]).split('或')[
                        0].strip()
...
def fetch_a_share_data_from_akshare(
    ...
    try:
        ...
        if interval in ["daily", "weekly", "monthly"]:
            if is_etf:
                df = ak.fund_etf_hist_em(
                    symbol=symbol, period=interval, start_date=start_date, end_date=end_date, adjust="qfq")
            else:
            ...
            df = ak.stock_zh_a_hist_min_em(
                symbol=symbol, start_date=date_str, end_date=date_str, period='1', adjust='qfq')
        ...
        final_cols = ['trade_date', 'open', 'high', 'low', 'close',
        ...
        logger.error(
            f"Failed to fetch or process data from AKShare for {stock_code}: {e}", exc_info=True)
        return pd.DataFrame()
