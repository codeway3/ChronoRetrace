...
def get_expiration_dates(symbol: str) -> tuple:
    ...
    try:
        ...
        logger.error(f"Failed to fetch expiration dates for {symbol}: {e}", exc_info=True)
        raise
    ...
    try:
        ...
        for col in ['last_trade_date']:
            if col in combined.columns:
                combined[col] = pd.to_datetime(combined[col]).dt.strftime('%Y-%m-%d %H:%M:%S')
        ...
        logger.error(f"Failed to fetch option chain for {symbol} on {expiration_date}: {e}", exc_info=True)
        raise
