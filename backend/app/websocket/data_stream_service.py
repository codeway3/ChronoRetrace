...
class DataStreamService:
    ...
    def __init__(self, connection_manager: ConnectionManager, redis_manager: RedisCacheManager):
    ...
    async def start_data_stream(self, topic: str) -> bool:
        ...
        try:
            ...
            # 创建数据流任务
            stream_task = asyncio.create_task(self._data_stream_worker(topic, topic_info))
                    ...
                    if data:
                        ...
                        if self._is_data_updated(topic, data):
                            ...
                            if topic in self.stream_stats:
                                ...
                                self.stream_stats[topic]["last_update"] = datetime.utcnow()
    ...
    async def _fetch_real_time_data(self, data_type: str, symbol: str, interval: str) -> Optional[Dict[str, Any]]:
        """
        获取实时数据
    ...
    async def _fetch_stock_data(self, symbol: str, interval: str) -> Optional[Dict[str, Any]]:
        """
        获取股票实时数据
        ...
        try:
            ...
            if data and len(data) > 0:
                ...
                formatted_data = {
                    ...
                    "timestamp": latest_data.get("timestamp", datetime.utcnow().isoformat())
                }
    ...
    async def _fetch_crypto_data(self, symbol: str, interval: str) -> Optional[Dict[str, Any]]:
        """
        获取加密货币实时数据
    ...
    async def _fetch_futures_data(self, symbol: str, interval: str) -> Optional[Dict[str, Any]]:
        """
        获取期货实时数据
    ...
    async def _push_data(self, topic: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            sent_count = await self.connection_manager.broadcast_to_topic(topic, message)
                    ...
                    if not subscribers:
                        ...
                        if topic in self.stream_stats:
                            ...
                            if last_update:
                                ...
                                if started_at:
                                    time_diff = (datetime.utcnow() - started_at).total_seconds()
                                    if time_diff > 300:
                                        inactive_topics.append(topic)
