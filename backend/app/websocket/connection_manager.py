...
class ConnectionManager:
        ...
        try:
            if client_id in self.active_connections:
                ...
                try:
                    ...
                    logger.warning(f"关闭客户端 {client_id} WebSocket连接时出错: {close_error}")
    ...
    async def broadcast_to_topic(self, topic: str, message: Dict[str, Any]) -> int:
        ...
        if success_count > 0:
            logger.debug(f"向主题 {topic} 广播消息，成功: {success_count}, 失败: {len(failed_clients)}")
    ...
    async def _heartbeat_monitor(self, client_id: str) -> None:
        ...
        try:
            while client_id in self.active_connections:
                ...
                if websocket.client_state.name != "CONNECTED":
                    logger.info(f"客户端 {client_id} WebSocket状态异常: {websocket.client_state.name}，停止心跳")
                    await self._cleanup_connection(client_id)
                    break
