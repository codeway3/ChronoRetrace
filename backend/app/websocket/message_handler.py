...
class MessageHandler:
    ...
    async def handle_message(self, client_id: str, message: str) -> None:
        ...
        try:
            ...
            if not isinstance(data, dict) or "type" not in data:
                await self._send_error(client_id, "invalid_message_format", "消息格式无效")
                return
            ...
            if handler:
                ...
                await self._send_error(client_id, "unknown_message_type", f"未知的消息类型: {message_type}")
    ...
    async def _handle_subscribe(self, client_id: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not self._validate_topic(topic):
                await self._send_error(client_id, "invalid_topic", f"无效的主题格式: {topic}")
                return
            ...
            if not success:
                await self._send_error(client_id, "subscribe_failed", f"订阅主题失败: {topic}")
    ...
    async def _handle_unsubscribe(self, client_id: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not success:
                await self._send_error(client_id, "unsubscribe_failed", f"取消订阅主题失败: {topic}")
                
        except Exception as e:
            logger.error(f"处理取消订阅消息时出错: {e}")
            await self._send_error(client_id, "unsubscribe_error", "取消订阅处理错误")
    
    async def _handle_heartbeat_response(self, client_id: str, data: Dict[str, Any]) -> None:
        """
        处理心跳响应消息
        ...
        try:
            ...
            if client_id in self.connection_manager.connection_metadata:
                self.connection_manager.connection_metadata[client_id]["last_heartbeat"] = datetime.utcnow()
    ...
    async def _handle_get_subscriptions(self, client_id: str, data: Dict[str, Any]) -> None:
        """
        处理获取订阅列表消息
        ...
        """
        try:
            subscriptions = list(self.connection_manager.client_subscriptions.get(client_id, set()))
            ...
            await self._send_error(client_id, "get_subscriptions_error", "获取订阅列表错误")
    ...
    def _validate_topic(self, topic: str) -> bool:
        ...
        try:
            ...
            # 支持的主题类型
            valid_types = ["stock", "crypto", "futures", "options", "commodity", "market"]
