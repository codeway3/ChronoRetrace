...
from app.services.crypto_fetcher import get_top_cryptos, get_crypto_ohlcv
...
async def read_top_cryptos():
    ...
    if not cryptos:
        raise HTTPException(status_code=404, detail="Could not fetch top cryptocurrencies.")
    return cryptos
...
async def read_crypto_history(
    ...
    if not history:
        raise HTTPException(status_code=404, detail=f"Could not fetch {interval} data for {symbol}.")
    return history
