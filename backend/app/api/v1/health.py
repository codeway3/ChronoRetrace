...
class HealthChecker:
    ...
    async def check_cache_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_status = "healthy" if cache_health and warming_health else "unhealthy"
    ...
    async def check_data_quality_health(self) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.base_url}/api/v1/data-quality/health",
                    timeout=5.0
                )
                if response.status_code == 200:
                    data = response.json()
            ...
            if isinstance(system_metrics, dict):
                ...
                return {
                    ...
                    "details": {
                        ...
                        "memory_usage_mb": getattr(system_metrics, 'memory_usage_mb', 0),
                        "cpu_usage_percent": getattr(system_metrics, 'cpu_usage_percent', 0)
                    }
                }
        except Exception as e:
