...
def get_asset_configs(
    ...
    configs = (
        query.order_by(AssetConfig.sort_order, AssetConfig.id)
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def create_asset_config(config: AssetConfigCreate, db: Session = Depends(get_db)):
    ...
    existing = (
        ...
        .filter(AssetConfig.asset_type == config.asset_type)
        .first()
    )
    if existing:
...
def update_asset_config(
    config_id: int, config: AssetConfigUpdate, db: Session = Depends(get_db)
):
    """更新资产配置"""
...
def update_asset_symbol(
    symbol_id: int, symbol: AssetSymbolUpdate, db: Session = Depends(get_db)
):
    """更新资产标的"""
...
def get_market_data(
    ...
    data = (
        query.order_by(AssetMarketData.trade_date.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def get_screener_templates(
    ...
    templates = (
        query.order_by(AssetScreenerTemplate.usage_count.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def create_screener_template(
    template: AssetScreenerTemplateCreate, db: Session = Depends(get_db)
):
    """创建筛选器模板"""
...
def get_backtest_templates(
    ...
    templates = (
        query.order_by(AssetBacktestTemplate.usage_count.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def create_backtest_template(
    template: AssetBacktestTemplateCreate, db: Session = Depends(get_db)
):
    """创建回测模板"""
