...
async def websocket_endpoint(websocket: WebSocket, client_id: str, token: Optional[str] = None):
    ...
    """
    logger.info(f"WebSocket连接请求: client_id={client_id}, token={'***' if token else 'None'}")
        ...
        while True:
            try:
                ...
                if websocket.client_state.name != "CONNECTED":
                    logger.warning(f"客户端 {client_id} WebSocket状态异常: {websocket.client_state.name}")
                    break
                ...
                try:
                    ...
                    logger.warning(f"客户端 {client_id} 发送了无效的JSON消息: {json_error}")
                ...
                # 检查是否是连接相关的错误
                connection_errors = [
                    "not connected", "closed", "connection", "broken pipe", 
                    "connection reset", "websocket is not connected"
                ]
...
async def get_websocket_stats():
    ...
    return {
        ...
        "message_types": message_handler.get_supported_message_types() if message_handler else [],
        "topic_examples": message_handler.get_topic_examples() if message_handler else {}
    }
