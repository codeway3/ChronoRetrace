...
from app.schemas.backtest import GridStrategyConfig, BacktestResult, GridStrategyOptimizeConfig, BacktestOptimizationResponse
...
def run_grid_optimization_api(
    ...
    try:
        ...
        logger.error(f"ValueError during optimization for {config.stock_code}: {e}", exc_info=True)
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(f"An unexpected error occurred during optimization for {config.stock_code}: {e}", exc_info=True)
