...
from app.schemas.corporate_action import CorporateActionInDB, CorporateActionResponse
...
async def get_all_stock_list_cached(
    ...
    """
    try:
        result = await stock_service.get_stock_list(db, market_type)
    except Exception as e:
        logger.exception("Error in get_all_stock_list_cached")
...
async def refresh_stock_list_cached(
    ...
    try:
        ...
        logger.exception("Error in refresh_stock_list_cached")
        raise HTTPException(status_code=500, detail=str(e)) from e
    else:
        return result
...
async def get_stock_data_cached(
    ...
    try:
        ...
        logger.exception("Error in get_stock_data_cached")
        raise HTTPException(status_code=500, detail=str(e)) from e
    else:
        return data
...
async def sync_data_for_symbol_cached(
    ...
    try:
        ...
        logger.exception("Error in sync_data_for_symbol_cached")
        raise HTTPException(status_code=500, detail=str(e)) from e
    else:
        return {
...
async def get_fundamental_data_cached(
    ...
    try:
        ...
        logger.exception("Error in get_fundamental_data_cached")
        raise HTTPException(status_code=500, detail=str(e)) from e
    else:
...
async def get_corporate_actions_cached(
    ...
    try:
        ...
        logger.exception("Error in get_corporate_actions_cached")
        raise HTTPException(status_code=500, detail=str(e)) from e
    else:
        converted_actions = [CorporateActionInDB.model_validate(a) for a in actions]
        return CorporateActionResponse(
            symbol=symbol, actions=cast("list[CorporateActionInDB]", converted_actions)
        )
...
async def get_annual_earnings_cached(
    ...
    try:
        ...
        logger.exception("Error in get_annual_earnings_cached")
        raise HTTPException(status_code=500, detail=str(e)) from e
    else:
        return earnings
...
async def invalidate_stock_cache(
    ...
    try:
        ...
        logger.exception("Error invalidating stock cache")
        raise HTTPException(status_code=500, detail=str(e)) from e
    else:
        return {
            "message": f"Cache invalidated for {stock_code}",
            "market_type": market_type,
        }
...
async def preload_hot_stocks(
    ...
    try:
        ...
        logger.exception("Error preloading hot stocks")
        raise HTTPException(status_code=500, detail=str(e)) from e
