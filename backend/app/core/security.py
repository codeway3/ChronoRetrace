...
def require_roles(required_roles: list[str]):
    ...
    def role_checker(current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
        user_roles = db.query(UserRole.name).join(
            UserRoleAssignment, UserRole.id == UserRoleAssignment.role_id
        ).filter(UserRoleAssignment.user_id == current_user.id).all()
...
def require_moderator(current_user: User = Depends(get_current_active_user), db: Session = Depends(get_db)):
    """需要版主权限"""
...
def validate_ip_whitelist(request: Request, whitelist: list[str] = None):
    ...
    client_ip = request.client.host if request.client and hasattr(request.client, 'host') else None
...
def check_user_permissions(user: User, resource: str, action: str, db: Session) -> bool:
    ...
    if admin_role:
        user_admin = db.query(UserRoleAssignment).filter(
